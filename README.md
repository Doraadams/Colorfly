![Header](images/header.png)

![Java 17](https://img.shields.io/badge/Language-Java%2017-9B599A.svg?style=flat)
![Environment: Client](https://img.shields.io/badge/Environment-Client-1976d2?style=flat)
[![Mod loader: Fabric]][fabric]
![Requires_Geckolib](https://img.shields.io/badge/Requires-Geckolib-brightgreen?style=flat)

## Mojang shouldn't have removed fireflies... Why not add them back, but better?

This mod adds the Colorfly , a new and improved Firefly entity for Minecraft. This isn't just a particle but rather a mob built from the ground up focused on ambiance and advanced interactions with the Vanilla Minecraft feeling.

Colorflies may also blink depending on the time and how they are feeling! Maybe if you are a bit bored you can try to figure out what they are saying... *hint hint*

You can also capture the Colorflies! Carefully hold a glass bottle and right click them to safely store them for keeping. Then, whenever you want, craft them into a Colorfly Lantern!

Colorflies will spawn in semi large groups in certain biomes, so be on the lookout for flowers or places bugs might be to find them!

*✨ It helps to search around at night since they will be luminescent ✨*

#### Keep an eye out for the rare Blue Ghost Colorfly! They seem to have recently settled into dark and forested regions...

## Screenshots

![Image1](images/1.png)\
![Image2](images/2.png)\
![Image3](images/3.png)

[fabric]: https://fabricmc.net
[Mod loader: Fabric]: https://img.shields.io/badge/modloader-Fabric-1976d2?style=flat-square&logo=data:image/png;base64,